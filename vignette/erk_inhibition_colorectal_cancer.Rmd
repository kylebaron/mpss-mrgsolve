...
```{r, message = FALSE}
mod <- mread_cache("mapk", "../model") %>% update(end  = 56)
...
e <- ev_seq(e, wait = 6, e) %>% as_tibble() %>% arrange(ID)
...
- __BRAF inhibitor__ - vemurafanib (VEMU) 950 mg BID
- Compartment 8
```{r}
dataV <- ev(amt=960, cmt=8, ii=0.5, addl=120)
```

- __ERK inhibitor__ - GCD-994 (GDC) 400 mg QD
- Compartment 12
```{r}
...
sims <- sims %>% mutate(object = map(object,as.data.frame))
```
...
sims <- mutate(
  sims, 
  out = future_map(object, sim, Vp = vp, Mod = mod)
)
```
...
re_run <- 
  ...
  select(label,object) %>%
  filter(label %in% c("GDC", "COBI+GDC")) %>% 
  mutate(out = future_map(object, sim, Vp = vp_select, Mod = mod)) %>%
  select(label, out) %>% 
  unnest()
```
